                ...
                <div class="card-body">
                    <table id="tOS" class="table table-hover table-borderless">
                        ...
                        <tr role="row">
                            ...
                            <th class="sorting">Data estimada</th>
                            <th class="sorting">Funcionário Designado</th>
                            <th class="sorting">Realizado por</th>
                            <th class="sorting">Solução</th>
                    ...
                    <form id="formView">
                        <fieldset disabled>
                            ...
                            <div class="row">
                                ...
                                <div class="form-group col-md-4">
                                    <label for="inputDateFinishView">Data em que foi realizado: </label>
                                    <input type="text" class="form-control" id="inputDateFinishView">
                                </div>
...
@section('js')
    <script>
                    ...
                    },
                    {
                        ...
                        render: function (data, type, row, meta) {
                            if (row.deleted_at != null) {
                                return '<span class="badge badge-secondary mr-1 ml-1">Cancelado</span>';
                            } else if (data === 'Pendente') {
                                return '<span class="badge badge-warning mr-1 ml-1">' + data + '</span>';
                            } else {
                                return '<span class="badge badge-success mr-1 ml-1">' + data + '</span>';
                            }
                        }
                    },
                    {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    data.map(_data => {
                        ...
                        let finish = new Date(_data.updated_at);

                        $("#inputProblemView").val(_data.problem);
                        ...
                        $("#inputDateFinishView").val(("0" + finish.getDate()).slice(-2) + "/" + ("0" + (finish.getMonth() + 1)).slice(-2) + "/" + finish.getFullYear());
                        $("#inputSolutionProblemView").val(_data.solution_problem);
                        if (_data.deleted_at != null) {
                            $("#inputStatusView").val("Cancelado");
                        } else {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    data.map(_data => {
                        ...
                        $("#inputLocaleEdit").val(_data.locale);
                        $("#inputWorkstationEdit").val(_data.workstation);
                        $("#inputRealizedDateEdit").val(date.getFullYear() + "-" + ("0" + (date.getMonth() + 1)).slice(-2) + "-" + ("0" + date.getDate()).slice(-2));
                        $("#inputSolverEmployeeEdit").val(_data.solver);
                        $("#inputSolutionProblemEdit").val(_data.solution_problem);

                        let opt;

                        for (let i = 0, len = $('#inputDesignatedEmployeeEdit option').length; i < len; i++) {
                            opt = $('#inputDesignatedEmployeeEdit option')[i];
                            if (opt.text === _data.designated) {
                                opt.setAttribute('selected', true);
                            }
                        }

                        for (let i = 0, len = $('#inputStatusEdit option').length; i < len; i++) {
                            opt = $('#inputStatusEdit option')[i];
                            if (opt.text == _data.status) {
                                opt.setAttribute('selected', true);
                            }
                        }
        ...
        function update(id) {
            let data = {
                realized_date: $("#inputRealizedDateEdit").val(),
                designated_employee: $("#inputDesignatedEmployeeEdit").val(),
                status_id: $("#inputStatusEdit").val(),
                solver_employee: $("#inputSolverEmployeeEdit").val(),
                solution_problem: $("#inputSolutionProblemEdit").val(),
            }
